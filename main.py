...
flags.DEFINE_integer("step1_num_epochs", 100,
                     "Number of times to run step1 train.")
flags.DEFINE_integer("step2_num_epochs", 5,
                     "Number of times to run step2 train.")
flags.DEFINE_integer("stepe2e_num_epochs", 5,
                     "Number of times to run stepe2e train.")
flags.DEFINE_string("save_path", './model/',
                     "Path to save.")
flags.DEFINE_integer("batchsize",2,
                     "bs")

def main(argv):
    device = torch.device("cuda:0")
    trainset = MHPdataset(root='G:/LV-MHP-v2/train/', mode='train', scale=True)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=FLAGS.batchsize, num_workers=12, shuffle=True)
    valset = MHPdataset(root='G:/LV-MHP-v2/val/', mode='val', scale=True)
        ...
        for phase in ['train', 'val']:
            if phase == 'train':
                for i,data in enumerate(trainloader):
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        outputs = bgfg_net(inputs)
                        seg_loss = dice_loss(outputs, segs)
                        a_seg_input=torch.cat([segs,outputs],0)
        ...
        for phase in ['train', 'val']:
            if phase == 'train':
                for i,data in enumerate(trainloader):
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        bgfgouts = bgfg_net(inputs)
                        bgfgouts=torch.cat([bgfgouts,inputs])
        ...
        for phase in ['train', 'val']:
            if phase == 'train':
                for i,data in enumerate(trainloader):
                    ...
                    parse=parse.to(device)
                    with torch.set_grad_enabled(phase == 'train'):
                        bgfgout = bgfg_net(inputs)
                        bgfgouts=torch.cat([bgfgout,inputs])
                        outputs = seg_net(bgfgouts)
                        a_parse_label = torch.cat([torch.ones(FLAGS.batchsize), torch.zeros(FLAGS.batchsize)], -1)
                        a_parse_inputt=torch.cat([segs,parse],1)
                        a_parse_inputf=torch.cat([bgfgout,outputs],1)
                        a_parse_input=torch.cat([a_parse_inputt,a_parse_inputf],0)
